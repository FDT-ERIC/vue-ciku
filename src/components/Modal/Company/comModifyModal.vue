<template>
  <div>
    <b-modal :id="id" size="xl" :title="title" hide-footer ref="my-modal" scrollable>
      <b-form @submit="onSubmit">
        <div class="row" style="margin-top:20px">
          <div class="col-6">
            ...
            >
              <b-form-group id="input-group-1" :label="item" label-for="input-1">
                <b-form-input
                  style="border-style:solid; border-color:#66CCFF;"
                  id="input-1"
                  v-model="company_data[item]"
                  value="abc"
                ></b-form-input>
              </b-form-group>
            </div>
...
export default {
  ...
  methods: {
    ...
    onSubmit(evt) {
      //   evt.preventDefault();
      // alert(JSON.stringify(this.company_data));
      this.$axios
        .get("/api/knowledge/database/company/update", {
          params: {
            name: this.title,
            update: this.company_data
          }
        })
        .then(res => {
          // this.DetailModal.company_data = res.data.res;
          console.log("啦啦啦222：", res.data.res)
          alert("修改成功", res.data.res);
          this.$refs["my-modal"].hide();
        });
      // console.log("啦啦啦：", this.title)
      // alert("修改成功");
      // this.$refs["my-modal"].hide();
      // // console.log(this.company_data);
      console.log(JSON.stringify(this.company_data))
    }
  },
  mounted() {
